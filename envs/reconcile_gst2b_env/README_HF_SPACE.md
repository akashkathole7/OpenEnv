...
🎤 [Slide deck (PDF)](https://github.com/akashkathole7/OpenEnv/raw/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/data/slides/reconcile_gst2b_pitch_deck.pdf) · [HTML version](https://htmlpreview.github.io/?https://github.com/akashkathole7/OpenEnv/blob/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/data/slides/reconcile_gst2b_pitch_deck.html) — 9-slide judge-readable summary of problem, environment, reward design, Day 1 results, and the FM4 + FM5 research findings.
...
![Day 1 SFT-only intermediate: trained Qwen3-4B after 375-step SFT (purple, 0.280) vs oracle, 6 red-team attacks, prompted Qwen2.5-3B baseline, and 0.45 red-team ceiling. Final shipped headline 0.305 charted in P3 GRPO panel below](https://raw.githubusercontent.com/akashkathole7/OpenEnv/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/data/figures/day1_baseline_comparison.png)

*Day 1 SFT-only intermediate result. Trained Qwen3-4B after 375-step SFT (purple, n=5 mean 0.280, min-max error bar 0.17 - 0.35) sits above the prompted Qwen2.5-3B baseline (blue, 0.18) and below the CI-enforced red-team ceiling (red dashed, 0.45). The same n=5 audit re-run after 100 P3 GRPO steps with Tier 2c length-shaping landed at **0.305 (the shipped Day 1 headline; +0.025 lift over this SFT-only intermediate)**, charted in the P3 GRPO panel below. The 6 red-team attacks (red bars) all score under 0.45 by design. Notably, the `query_only` attack (0.349) scores ABOVE the SFT-only Mode A marking trajectories, the reward-landscape inversion documented as Failure Mode 5 in [LESSONS_LEARNED.md](https://github.com/akashkathole7/OpenEnv/blob/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/LESSONS_LEARNED.md) §1, which is exactly why the P3 Tier 2c mitigation was designed. Source: live-recomputed from `data/audit_F_n5.json` and `tests/envs/test_reconcile_gst2b_reward_hacking.py` via [`scripts/make_day1_figure.py`](https://github.com/akashkathole7/OpenEnv/blob/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/scripts/make_day1_figure.py). The same chart is rendered live as Tab 4 in this Space, with the trained bar at the post-GRPO 0.305 value.*
...
4. **Phase 3 GRPO with Tier 2c length-shaping** (~100 min wall, A100). [`train_grpo_real.py`](https://github.com/akashkathole7/OpenEnv/blob/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/scripts/train_grpo_real.py) on `sft_checkpoint/merged` for 100 GRPO steps. Tier 2c bonus: `+0.10` if `n_marks >= 10` AND `distinct_verbs >= 4` (gates designed so no red-team attack triggers; all 6 attacks use ≤3 distinct verbs, well below the ≥4 threshold). n=5 audit on the post-GRPO checkpoint at the same sampling parameters lifts mean composite reward from 0.280 to **0.305 (the shipped Day 1 headline; +0.025 lift)**. Headline data: [`data/audit_grpo_p3_F_n5.json`](https://github.com/akashkathole7/OpenEnv/blob/scaffold/reconcile-gst2b/envs/reconcile_gst2b_env/data/audit_grpo_p3_F_n5.json). The mitigation lifts Mode A marking-trajectory absolute scores but does not flip the 2/5 Mode B `query_only` seeds; full bimodal flip is post-hackathon agenda.
...
- **Failure Mode 5: reward-landscape inversion at Phase 2 → Phase 3 boundary.** The same arithmetic-reward + multi-component-clamping design that makes the 6 red-team attacks defensible (the central claim of this submission) also creates an exploit-favoring asymmetry at this policy quality. Partially mitigated by P3 Tier 2c length-shaping bonus (+0.025 lift to 0.305); full bimodal flip remains post-hackathon agenda (stronger shaping magnitudes or class-balanced SFT subsampling).
