---
title: Reconcile Gst2b Env Environment Server
emoji: 🥊
colorFrom: green
colorTo: purple
sdk: docker
...
try:
    ...
    reconcile_gst2b_envenv = ReconcileGST2BEnv.from_docker_image("reconcile_gst2b_env-env:latest")
    ...
    # Always clean up
    reconcile_gst2b_envenv.close()
```

That's it! The `ReconcileGST2BEnv.from_docker_image()` method handles:
...
```bash
# From project root
docker build -t reconcile_gst2b_env-env:latest -f server/Dockerfile .
```
...
If you already have a Reconcile Gst2b Env environment server running, you can connect directly:

```python
from reconcile_gst2b_env import ReconcileGST2BEnv

# Connect to existing server
reconcile_gst2b_envenv = ReconcileGST2BEnv(base_url="<ENV_HTTP_URL_HERE>")
...
python3 server/reconcile_gst2b_environment.py
```
...
├── client.py              # ReconcileGST2BEnv client
├── models.py              # Action and Observation models
└── server/
    ├── __init__.py        # Server module exports
    ├── reconcile_gst2b_environment.py  # Core environment logic
