...
The environment is correctly hard, and the reward design surfaces that hardness as three distinct structural failure modes across the Qwen3 family. **Qwen3-0.6B** (`data/smoke_test_10step.json`): bimodal tool-call rate around 0.25 per rollout; can't reliably chain 5 mark_* actions out of coin-flip^5. Pins R1/R2 at the 0.01 floor, eval plateaus at 0.353. **Qwen3-1.7B** (`data/training_log_qwen3_1_7b_partial.json`): tool-call rate collapsed to 0.00 across 15 training steps, entropy down to 0.12 from 0.6B's 0.44, deterministic non-tool-call output mode, confident but wrong. **Qwen3-4B** (`data/training_log_qwen3_4b_partial.json`): opposite problem, emits parseable tool-call JSON on every turn but never commits to a label, over-queries until the 50-step env budget exhausts, crashing R4 to 0.01 (baseline eval total 0.255). Three failure modes, three different reward components pinned to floor. No model in this range passes the 0.45 red-team ceiling by accident. The red-team battery holds unchanged across all attempts (`submit_all_matched=0.308`, `query_only=0.349`, all six under 0.40). Real fix needs tool-SFT warm-start (teach mark_* preference over query before GRPO) or a model with stronger label-vs-query prior (Qwen3-8B+, GPT-OSS-20B, Claude) on compute that permits it (A100 × 4 h, stated next ask in `EXEC_SUMMARY.md` line 4).

### Day 1 on-site, Bangalore, 2026-04-25, A100 SXM4-80GB

The compute ask in the previous paragraph landed. Day 1 ran a full Phase 2 SFT on Qwen3-4B (375 optimizer steps, 31 minutes wall time, train_loss 0.341 aggregate / final per-step 0.207, monotonic descent; full curve in [`data/sft_full_run.log`](data/sft_full_run.log)). Two new failure modes surfaced past Phase 2, and they are the substantive findings of this on-site:

**Failure Mode 4: audit-script OOD trap chain.** Diagnosing the SFT checkpoint required exhausting four sequential bugs in the audit infrastructure before observing real model behavior. Greedy decoding masked the policy under sampling. Fresh-context per-turn prompts violated the multi-turn accumulation pattern of the SFT training data. A `[:600]` truncation false-alarm cost ~5 minutes of misdirected investigation. And finally, the load-bearing one: the missing `tools=` parameter to `apply_chat_template` activates a structurally different output mode in Qwen3's chat template. Without it, the model emits zero marks across n=5 seeds (apparent collapse). With it, the same checkpoint emits 32-46 marks per 50-step trajectory. The diagnostic tool is committed at [`scripts/audit_sft_rollout_quality.py`](scripts/audit_sft_rollout_quality.py); see [LESSONS_LEARNED.md](LESSONS_LEARNED.md) §1 for the full chain. The lesson: the diagnostic infrastructure is as load-bearing as the model itself; small audit details (decoder mode, context accumulation, chat-template parameters) can fabricate failure modes that look identical to real ones.

**Failure Mode 5: reward-landscape inversion at the Phase 2 → Phase 3 boundary.** With `tools=` enabled, the n=5 audit revealed a **bimodal policy**. 3 of 5 seeds emit marks; sample breakdown (seed 9031, 50 steps, step_budget_exhausted): R1 = 0.016, R2 = 0.034, R3 = 0.99, R4 = 0.01, total 0.2634. R3 saturates at 0.99 because queries-before-submit clear the Rule 36(4) precondition; R4 floors at 0.01 from full-budget exhaust. Total range across three Mode A seeds: 0.17 - 0.26. The other 2 of 5 seeds collapse to a 3-step `get_schema → list_gstins → submit` shape (R3 = 0.99, R4 = 0.99, R1 = R2 = 0.01 floor, total 0.353). **The query_only Mode B scores higher than any marking Mode A trajectory.** GRPO's advantage signal would push the model toward the attack within ~50 steps, matching the 0.353 query_only attack plateau documented in [`tests/envs/test_reconcile_gst2b_reward_hacking.py`](../../tests/envs/test_reconcile_gst2b_reward_hacking.py); a structurally distinct failure from the 1.7B submit-shortcut, but converging to the same composite reward via R3 + R4 saturation. Phase 3 GRPO ran with a Tier 2c length-shaping bonus added to `train_grpo_real.py`'s `reward_func` (NOT to `rewards.py`, Guardrail 1 frozen). 100 steps lifted the n=5 mean from 0.280 to 0.305 (+0.025); the 2/5 Mode B query_only seeds did not flip. The same arithmetic-reward + multi-component-clamping design that makes the 6 red-team attacks defensible (the central claim of this submission) also creates this asymmetry. Stronger shaping magnitudes or class-balanced SFT subsampling are post-hackathon work.

**Day 1 ships an honest n=5 mean composite reward of 0.305** post P3 GRPO with Tier 2c length-shaping bonus (above SFT-only baseline 0.280, above prompted Qwen2.5-3B baseline 0.18, below red-team ceiling 0.45). Full diagnostic chain artifacts at [`data/audit_seeds_9031_9034.json`](data/audit_seeds_9031_9034.json) (pre-fix collapse), [`data/audit_F_n5.json`](data/audit_F_n5.json) (SFT n=5 with `tools=` fix), [`data/audit_step100_n5.json`](data/audit_step100_n5.json) (early-stop control), [`data/audit_grpo_p3_F_n5.json`](data/audit_grpo_p3_F_n5.json) (post-GRPO headline). The P3 mitigation lifts the bimodal Mode A scores by ~0.05 (from 0.26 ceiling toward 0.30 average) without flipping the 2/5 Mode B query_only seeds; full bimodal redistribution requires stronger shaping magnitudes (post-hackathon agenda). The reward-landscape-inversion finding remains the central design observation: shape one component without sacrificing the red-team CI guarantees of the others. Empirical FM4 confirmation across training stages: the fresh-context eval callback in `train_grpo_real.py:_eval_episode` reported a flat 0.354 across all 100 GRPO steps, identical to the step-0 baseline, masking ALL training progress (visualized in [`data/figures/grpo_reward_curve.png`](data/figures/grpo_reward_curve.png)).
