...
The environment is correctly hard. R1 is macro-F1 across 5 labels; to register non-zero signal the agent must chain ≥5 valid `<tool_call>` blocks inside the budget. At ~25% tool-call rate per rollout, that's coin-flip^5 — no recipe fixes that at 0.6B scale. Scaling up to Qwen3-1.7B made it strictly worse: `data/training_log_qwen3_1_7b_partial.json` records 15 steps on T4 at tool-call-frequency **zero** across the board, with entropy collapsed to ~0.12 (vs 0.44 on 0.6B) — the policy converged to a deterministic non-tool-call output mode, confident but wrong. Kill-shot on the "bigger model emits more tool calls" hypothesis at this parameter range. The red-team battery holds unchanged across all attempts (`submit_all_matched=0.308`, `query_only=0.349`, all six under 0.40). Real fix needs a larger model with a stronger pre-training tool-calling prior (Qwen3-4B on T4x2 pipeline-parallel is the next attempt — see `EXEC_SUMMARY.md` line 10) or tool-SFT warm-start before GRPO.
