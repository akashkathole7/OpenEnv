...
## 7. What we learned from a failed training run

The first attempt at real GRPO — Qwen3-0.6B + LoRA rank 16, 150 steps on Kaggle T4 — collapsed. `curves_150step_backup.json` shows a flat 0.353 at every eval point (steps 0/25/50/75/100/125/150), with R1=R2 pinned at the 0.01 floor and R3=R4 pinned at 0.99. That reward is within 0.004 of `test_attack_query_only` (~0.349) — i.e. training converged to a policy my own red-team test suite documents as an attack signature. A judge who runs `pytest tests/envs/test_reconcile_gst2b_reward_hacking.py -v -s` and overlays it on the training curve sees the match instantly. Not shippable; worth understanding.

Three-angle diagnosis. (i) Qwen3's chat template defaults `enable_thinking=True`, which inserts a `<think>...</think>` CoT block before the assistant answer. On a 512-token completion budget, the think block ate the entire budget before any `<tool_call>` was emitted — training logs showed `completions/clipped_ratio=1.0` and `completions/mean_terminated_length=0`. (ii) TRL GRPOConfig's default `beta=0.04` KL-to-reference-model coefficient kept the LoRA tethered to the base model, actively cancelling the rare updates from non-zero-std groups. (iii) Without successful tool calls, every rollout in a 4-generation group landed in the same "no mutations emitted" reward bucket → `reward_std=0` → GRPO advantage=0 → `grad_norm=0` → no policy update. Zero-variance advantage on most steps is why 150 steps of training looked like zero steps of training.

The fix (commit `3ca024d`) is three-layered. Tier 1 pins the generation config: `temperature=0.7`, `top_p=0.95`, `top_k=20`, `repetition_penalty=1.1`, `max_completion_length=448`, `beta=0.0` (KL tether off), and a tokenizer monkeypatch forcing `enable_thinking=False` on every `apply_chat_template` call so TRL's rollout generation and my own eval loop both bypass the think preamble. Tier 2 adds two reward-shaping wrappers inside `train_grpo_real.py` — zero-std jitter (σ=0.005 gaussian noise injected when all 4 group rewards are identical, seeded by a deterministic monotonic counter for bit-identical reruns) and a +0.02 format bonus when `env._trajectory` has ≥1 action. Neither touches `rewards.py`, so the red-team ceilings at 0.45 still hold — re-ran the 6-attack suite with max `query_only = 0.349`, unchanged from baseline. Rollback is one `git revert HEAD`.

The 10-step smoke test (`data/smoke_test_10step.json`) confirms the fixes land mechanically: `reward_std > 0` on every step (min 0.002, max 0.13), `grad_norm > 0` on every step (0.48–0.55), `frac_reward_zero_std = 0`. The remaining bottleneck is model capacity. Qwen3-0.6B emits parseable tool-call JSON on only 5 of the 10 steps at a 448-token budget; the other 5 ramble until clip with zero tool calls. That's a coin-flip signal — 150 steps of GRPO on a coin-flip won't converge no matter how tuned the recipe is. The honest next step is to scale to a 3B model on an A100 40GB for ~4 hours (see `EXEC_SUMMARY.md` line 10). Shipping the environment, the diagnosis, the fix commit, and the smoke evidence — rather than a fabricated "smooth" curve — is the right answer.
